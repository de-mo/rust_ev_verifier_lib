...
use super::number_theory::{is_small_prime, satisfy_euler_criterion};
...
pub fn get_small_prime_group_members(
    ...
    while res.len() < desired_number && BigUint::from(current) < *p && current < usize::pow(2, 31) {
        if satisfy_euler_criterion(current, &p) && is_small_prime(current).unwrap() {
            res.push(current);
        }
        current += 2;
    }
...
mod test {
    ...
    fn test_get_small_prime_group_members() {
        ...
        assert_eq!(
            get_small_prime_group_members(&p, 3).unwrap(),
            vec![5, 17, 19,]
        );
    }
}
